...
class LedControlVC: UIViewController, BluetoothSerialDelegate {
    ...
    override func viewDidLoad() {
        ...
        circularSliderView.setCircluarRingShadow(shadowColor: UIColor.green, radius: 3)
        circularSliderView.setArrayValues(labelValues: arrValues, currentIndex: 0)
        circularSliderView.setKnobOfSlider(knobSize: 40, knonbImage: UIImage(named: "barometer")!)
        circularSliderView.setCircularRingWidth(innerRingWidth: 18, outerRingWidth: 30)
        circularSliderView.setValueTextFieldDelegate(viewController: self)
        circularSliderView.setBackgroundColorOfAllButtons(startPointColor: UIColor.flatGreen(), endPointColor: UIColor.flatGreen(), knobColor: UIColor.flatGreen())
...
//        Power
    ...
    @objc func imageTapped(tapGestureRecognizer: UITapGestureRecognizer){
        ...
        if powerCheckBool{
            ...
            let brightnessString = "FF0A010101"
//            var brightnessByteArray = self.toByteArray(brightnessString)
//            brightnessByteArray.append(contentsOf: self.toByteArray("\(circularSliderView!.currentIndex)"))
...
//            serial.sendBytesToDevice(brightnessByteArray)
    ...
    @IBAction func pinButtonPressed(_ sender: Any) {
        let alertVC = UIAlertController(title: "Choose Pin", message: "Brightness value is supported only on pins 2 to 13, 44 to 46", preferredStyle: .alert)
        ...
        let okAction = UIAlertAction(title: "Select", style: .default) { action in
            if let textField = alertVC.textFields?.first,
                ...
                pinByteArray.append(contentsOf: self.toByteArray(pinValue))
